...
jQuery(document).ready(function(){
	...
	if (Tagger.TaggerField.find('.SingleTagsInput').length > 0){
		...
		Tagger.TaggerField.find('#SingleTagsInput_'+Tagger.FieldID).tagsInput({
			...
			autocomplete: {autoFocus:false, open:function(){ Tagger.TaggerField.find('#SingleTagsInput_'+Tagger.FieldID+'_tag').autocomplete("widget").width(300).addClass('DDTaggerAC') }},
			unique:true
		});
...
Tagger.InstantSearch = function(event){
	if (event.which == 13)	{
		...
		Tagger.TaggerField.find('.InstantInsert').autocomplete('close');
		return false;
	}
};
