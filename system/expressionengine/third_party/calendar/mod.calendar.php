...
class Calendar extends Module_builder_calendar
{
	...
	public function calendars()
	{
		...
		//$this->EE->TMPL->log_item('Calendar: Processing parameters');
			...
			$this->P->value('calendar_name') != '')
		{
			...
			if ( empty( $ids ) )
			{
				//$this->EE->TMPL->log_item('Calendar: No results for
		...
		{
			//$this->EE->TMPL->log_item('Calendar: No results, bailing');
			return $this->no_results();
		}
		...
		if ($this->P->value('date_range_start') !== FALSE OR
			$this->P->value('date_range_end') !== FALSE)
		{
			//$this->EE->TMPL->log_item('Calendar: Calculating date ranges');
		...
		{
//$this->EE->TMPL->log_item('Calendar: No calendars, bailing');
			return $this->no_results();
		}
		...
		$this->EE->TMPL->tagparams['entry_id'] = implode('|', $calendar_array);
		$this->EE->TMPL->tagparams['weblog'] = CALENDAR_CALENDARS_CHANNEL_NAME;
		$this->EE->TMPL->tagparams['channel'] = CALENDAR_CALENDARS_CHANNEL_NAME;
		...
		{
//$this->EE->TMPL->log_item('Calendar: Channel module says no results, bailing');
			return $this->no_results();
		}
		...
		if ($this->EE->extensions->active_hook('calendar_calendars_channel_query') === TRUE)
		{
			$channel->query = $this->EE->extensions->call('calendar_calendars_channel_query', $channel->query, $calendar_array);
		...
		//$this->EE->TMPL->log_item('Calendar: Adding Calendar variables');
		...
		//$this->EE->TMPL->log_item('Calendar: Parsing, via channel module');
		...
		//$this->EE->TMPL->log_item('Calendar: Parsing related entries, via Weblog module');
		...
		//$this->EE->TMPL->log_item('Calendar: Done!');
	...
	public function events()
	{
		...
		//default off.
		if ( $this->check_yes( $this->EE->TMPL->fetch_param('dynamic') ) )
		{
			$this->EE->TMPL->tagparams['dynamic'] 	= (APP_VER < 2.0 ) ? 'on' : 'yes';
		}
		else
		{
		...
		*/
		...
		//$this->EE->TMPL->log_item('Calendar: Fetching events');

		$ids = $this->data->fetch_event_ids($this->P);

		/*$category = FALSE;

		if (FALSE AND isset($this->EE->TMPL) AND
			 is_object($this->EE->TMPL) AND
			 $this->EE->TMPL->fetch_param('category') !== FALSE AND
			 $this->EE->TMPL->fetch_param('category') != ''
		)
		{
			$category = $this->EE->TMPL->fetch_param('category');

			unset($this->EE->TMPL->tagparams['category']);
		}
		...
		if (empty($ids))
		{
			//$this->EE->TMPL->log_item('Calendar: No events, bailing');
			return $this->no_results();
		}
		...
		if ($this->EE->extensions->active_hook('calendar_events_event_ids') === TRUE)
		{
			$ids = $this->EE->extensions->call('calendar_events_event_ids', $ids);
			if ($this->EE->extensions->end_script === TRUE) return;
		}
		//
		// -------------------------------------------
		...
		$this->EE->TMPL->var_single['entry_id'] = 'entry_id';
		...
		foreach ($this->EE->TMPL->var_pair as $name => $params)
		{
			...
			foreach ($var_pairs as $pair)
			{
				...
				$this->EE->TMPL->var_pair[] 					= $this->EE->TMPL->var_pair[$name];
				$this->EE->TMPL->var_pair[$new_name]['id'] 	= '';
				// Leave the old name behind so we can pick it up later and use it
				$this->EE->TMPL->var_pair[$pair] 			= $name;

				unset($this->EE->TMPL->var_pair[$name]);
			}
		}
		...
		foreach ($this->EE->TMPL->var_single as $k => $v)
		{
		...
		$this->EE->TMPL->tagparams['entry_id'] 			= implode('|', array_keys($ids));

		//if we have event names, lets set them for the URL title
		if ( ! in_array($this->EE->TMPL->fetch_param('event_name'), array(FALSE, ''), TRUE))
		{
			$this->EE->TMPL->tagparams['url_title'] = $this->EE->TMPL->fetch_param('event_name');
		}

		$this->EE->TMPL->tagparams[$this->sc->channel] 	= CALENDAR_EVENTS_CHANNEL_NAME;
...
//$this->EE->TMPL->log_item('Calendar: Channel module says no results, bailing');
		...
		if ($this->EE->extensions->active_hook('calendar_events_channel_query') === TRUE)
		{
			$channel->query = $this->EE->extensions->call('calendar_events_channel_query', $channel->query, $ids);
			if ($this->EE->extensions->end_script === TRUE) return;
		}
		//
		// -------------------------------------------
		...
		//$this->EE->TMPL->log_item('Calendar: Fetching Calendar_event class');
		...
		//$this->EE->TMPL->log_item('Calendar: Creating events');
		...
		//$this->EE->TMPL->log_item('Calendar: Adding variables to channel query result');
		...
		{
//$this->EE->TMPL->log_item('Calendar: Weblog query is empty, bailing');
			return $this->no_results();
		}
		...
		{
			$this->EE->TMPL->log_item('Calendar: Reordering');
			...
			$this->event_timeframe_total > $this->P->value('event_limit'))
		{
			...
			$pagination_data = $this->universal_pagination(array(
				...
				'tagdata'				=> $this->EE->TMPL->tagdata . $this->paginate_tagpair_data,
			...
			if ($this->EE->extensions->active_hook('calendar_events_create_pagination') === TRUE)
			{
				$pagination_data = $this->EE->extensions->call(
		...
		$this->EE->TMPL->tagdata = str_replace('occurrence_count', $occurrence_hash, $this->EE->TMPL->tagdata);
		...
		//$this->EE->TMPL->log_item('Calendar: Parsing variable pairs');

		foreach ($var_pairs as $var)
		{
			if (isset($this->EE->TMPL->var_pair[$var]))
			{
				// -------------------------------------
				...
				foreach ($events as $k => $data)
				{
					...
					{
						$var_tag = (! is_array($this->EE->TMPL->var_pair[$var])) ? $this->EE->TMPL->var_pair[$var] : $var;
		...
		//$this->EE->TMPL->log_item('Calendar: Parsing related entries, via channel module');
	...
	public function occurrences()
	{
		...
		$this->EE->TMPL->tagdata = LD.'occurrences'.RD.$this->EE->TMPL->tagdata.LD.T_SLASH.'occurrences'.RD;
		$this->EE->TMPL->var_pair['occurrences'] = FALSE;

		//$this->EE->TMPL->log_item('Calendar: occurrences() method handing off to events() method');
	...
	public function day()
	{
		...
		if (strpos($this->EE->TMPL->tagdata, LD.'events'.RD) !== FALSE)
		{
			$this->EE->TMPL->var_pair['events'] = TRUE;
		}

		if (strpos($this->EE->TMPL->tagdata, LD.'display_each_hour'.RD) !== FALSE)
		{
			$this->EE->TMPL->var_pair['display_each_hour'] = TRUE;
		}

		if (strpos($this->EE->TMPL->tagdata, LD.'display_each_day'.RD) !== FALSE)
		{
			$this->EE->TMPL->var_pair['display_each_day'] = TRUE;
		}
...
//$this->EE->TMPL->log_item('Calendar: Processing parameters');
		...
		$replace = array(	$this->EE->TMPL->tagdata,
							LD.T_SLASH,
							LD,
							RD
							);
		$this->EE->TMPL->tagdata = str_replace($find, $replace, $this->view('day.html', array(), TRUE, $this->sc->addon_theme_path.'templates/day.html'));
	...
	public function week()
	{
		...
		if (strpos($this->EE->TMPL->tagdata, LD.'display_each_week'.RD) !== FALSE)
		{
			$this->EE->TMPL->var_pair['display_each_week'] = TRUE;
		}
	...
	public function month()
	{
		...
		if ($this->P->value('date_range_start') === FALSE)
		{
			if ($this->P->value('start_date_segment') AND $this->EE->uri->segment($this->P->value('start_date_segment')) AND strstr($this->EE->uri->segment($this->P->value('start_date_segment')), '-'))
			{
				list($year, $month) = explode('-', $this->EE->uri->segment($this->P->value('start_date_segment')));
		...
		$this->EE->TMPL->tagdata = str_replace($find, $replace, $this->view('month.html', array(), TRUE, $this->sc->addon_theme_path.'templates/month.html'));
		...
		if (strpos($this->EE->TMPL->tagdata, LD.'display_each_month'.RD) !== FALSE)
		{
			$this->EE->TMPL->var_pair['display_each_month'] = TRUE;
		}

		if (strpos($this->EE->TMPL->tagdata, LD.'display_each_day_of_week'.RD) !== FALSE)
		{
			$this->EE->TMPL->var_pair['display_each_day_of_week'] = TRUE;
		}
	...
	public function mini()
	{
		...
		if ($this->P->value('date_range_start') === FALSE)
		{
			if ($this->P->value('start_date_segment') AND
				$this->EE->uri->segment($this->P->value('start_date_segment')) AND
				strstr($this->EE->uri->segment($this->P->value('start_date_segment')), '-'))
			{
				list($year, $month) = explode('-', $this->EE->uri->segment($this->P->value('start_date_segment')));
				$this->P->params['date_range_start']['value'] = $this->CDT->change_date($year, $month, 1);
			}
	...
	public function form($return_form = FALSE, $captcha = '')
	{
		...
		{
			//$this->EE->TMPL->log_item('Calendar: This is an edited occurrence');

			$this->EE->TMPL->tagdata .= '<input type="hidden" name="entry_id" value="" />'."\n";
			$this->EE->TMPL->tagdata .= '<input type="hidden" name="calendar_parent_entry_id" value="{entry_id}" />'."\n";
			$this->EE->TMPL->tagdata .= '<input type="hidden" name="event_id" value="'.$event_data['event_id'].'" />'."\n";
			$this->EE->TMPL->tagdata .= '<input type="hidden" name="start_date" value="'.$this->P->value('occurrence_date', 'ymd').'" />'."\n";
			$this->EE->TMPL->tagdata .= '<input type="hidden" name="start_time" value="'.$event_data['start_time'].'" />'."\n";
			$this->EE->TMPL->tagdata .= '<input type="hidden" name="end_time" value="'.$event_data['end_time'].'" />'."\n";
			$this->EE->TMPL->var_single['entry_id'] = 'entry_id';
		...
		if (strpos($this->EE->TMPL->tagdata, LD.'calendar_date_widget'.RD) !== FALSE)
		{
			$this->EE->TMPL->tagdata = str_replace(LD.'calendar_date_widget'.RD, $this->date_widget($event_data), $this->EE->TMPL->tagdata);
		}
		...
		$this->EE->TMPL->tagparams['entry_id']	= $event_id;
		$this->EE->TMPL->tagparams['weblog']		= CALENDAR_EVENTS_CHANNEL_NAME;
		$this->EE->TMPL->tagparams['channel']	= CALENDAR_EVENTS_CHANNEL_NAME;
		...
		if ($edit === TRUE)
		{
			//$this->EE->TMPL->log_item('Calendar: Editing, so doing Weblog module tasks');
	...
	//$this->EE->TMPL->log_item('Calendar: Channel module says no results, bailing');
			...
			{
				$this->EE->TMPL->tagdata = str_replace(LD.$field, LD.$gibberish.$field, $this->EE->TMPL->tagdata);
			}
			...
			$this->EE->TMPL->tagdata = $channel->return_data;

			// -------------------------------------
			//  Add some hidden variables
			// -------------------------------------

			$date = $this->EE->localize->decode_date('%Y-%m-%d %g:%i %A', $super_temp_fake['entry_date']);
		...
		else
		{
			...
			$this->EE->TMPL->tagdata = str_replace(array_keys($fields), $fields, $this->EE->TMPL->tagdata);
		}
		...
		if (strpos($this->EE->TMPL->tagdata, LD.'calendar_') !== FALSE)
		{
			preg_match_all('#'.LD.'calendar_(.*?)'.RD.'#', $this->EE->TMPL->tagdata, $matches);
			if (! empty($matches))
			{
				foreach ($matches[0] as $k => $match)
				{
					$this->EE->TMPL->tagdata = str_replace($match, '', $this->EE->TMPL->tagdata);
				}
			}
		}

		if (strpos($this->EE->TMPL->tagdata, LD.'event_') !== FALSE)
		{
			preg_match_all('#'.LD.'(event_.*?)'.RD.'#', $this->EE->TMPL->tagdata, $matches);
			if (! empty($matches))
			{
				foreach ($matches[0] as $k => $match)
				{
	...
	public function date_widget($data = array())
	{
		...
		$id = $this->EE->TMPL->fetch_param('event_id');
		...
		if (APP_VER >= 2.0)
		{
			...
			))
			{
				return $this->EE->output->show_user_error(
					 'general', array(lang('invalid_calendar_permissions'))
				);
			}
		}
	...
	*/
	...
	public function icalendar()
	{
		...
		//$this->EE->TMPL->log_item('Calendar: Preparing tagdata');

		$this->EE->TMPL->tagdata =	implode($s, array(
			LD . 'event_title' . RD,
			LD . 'event_id' . RD,
		...
		$tvars 					= $this->EE->functions->assign_variables(
			$this->EE->TMPL->tagdata
		);
		$this->EE->TMPL->var_single 	= $tvars['var_single'];
		$this->EE->TMPL->var_pair 	= $tvars['var_pair'];
		...
		//$this->EE->TMPL->log_item('Calendar: Firing up Events()');

		$tagdata = $this->EE->TMPL->advanced_conditionals($this->events());

		// -------------------------------------
		//  Collect the events
		// -------------------------------------

		//$this->EE->TMPL->log_item('Calendar: Collecting events');
		...
		//$this->EE->TMPL->log_item('Calendar: Starting iCalCreator');
		...
		//$ICAL->setConfig('unique_id', parse_url($this->EE->config->item('site_url'), PHP_URL_HOST));
		$host = parse_url($this->EE->config->item('site_url'), PHP_URL_HOST);


		$vars = array_values($vars);

		//$this->EE->TMPL->log_item('Calendar: Iterating through the events');
		...
		foreach ($events as $key => $event)
		{
			...
			if ($key == 0)
			{
				...
				if ($this->P->value('calendar_id') != '')
				{
					...
					$cal_tz_query = $this->EE->db->query($sql);
			...
			if ( ! empty($odata))
			{
				$query = $this->EE->db->query(
					"SELECT *
					 ...
					 WHERE	event_id = " . $this->EE->db->escape_str($id)
				);
	...
	public function ics_update()
	{
		...
		{
			//$this->EE->TMPL->log_item('Calendar: It is not time to update icalendar data yet.');
			return;
		}
		elseif ($this->P->value('time_range_end', 'time') !== FALSE AND $this->P->value('time_range_end', 'time') < $time)
		{
			//$this->EE->TMPL->log_item('Calendar: The time to update icalendar data has passed.');
			return;
		}
		...
		if (empty($ids))
		{
			//$this->EE->TMPL->log_item('Calendar: Nobody is due for an update');
			return;
		}
...
//$this->EE->TMPL->log_item('Calendar: Fetching data');
	...
	{
		$this->actions();
		return $this->actions->datepicker_js( ! $this->check_no($this->EE->TMPL->fetch_param('include_jqui')));
	}
	/* END datepicker_js() */
	...
	{
		$this->actions();
		return $this->actions->datepicker_css( ! $this->check_no($this->EE->TMPL->fetch_param('include_jqui')));
	}
	/* END datepicker_css() */
...
//$this->EE->TMPL->log_item('Calendar: Looping');
		...
		do {
			...
			$output .= $this->swap_vars($vars, $this->EE->TMPL->tagdata);
...
//$this->EE->TMPL->log_item('Calendar: Output time');
		...
		return $this->swap_vars($vars, $this->EE->TMPL->tagdata);
	}
	// END date()
	...
	{
		//$this->EE->TMPL->log_item('Calendar: Building calendar output');
		...
		//$this->EE->TMPL->log_item('Calendar: Collecting tagdata');
		...
		if (isset($this->EE->TMPL->var_pair['display_each_day_of_week']))
		{
			preg_match(
		...
		if (isset($this->EE->TMPL->var_pair['events']))
		{
			preg_match(
			...
			if (isset($match[1]))
			{
				...
				$this->EE->TMPL->tagdata = $each_event;
				$output_at 			= 'event';
			}
		}

		if (isset($this->EE->TMPL->var_pair['display_each_hour']))
		{
			preg_match(
		...
		if (isset($this->EE->TMPL->var_pair['display_each_day']))
		{
			preg_match(
		...
		if (isset($this->EE->TMPL->var_pair['display_each_week']))
		{
			preg_match(
		...
		if (isset($this->EE->TMPL->var_pair['display_each_month']))
		{
			preg_match(
		...
		if (isset($this->EE->TMPL->var_pair['display_each_year']))
		{
			preg_match(
		...
		//$this->EE->TMPL->log_item('Calendar: Date range start: '. $this->P->value('date_range_start', 'ymd'));

		//$this->EE->TMPL->log_item('Calendar: Date range end: '. $this->P->value('date_range_end', 'ymd'));
		...
		//$this->EE->TMPL->log_item('Calendar:build_calendar() Fetching events. ' . count( $ids ) . ' events were found.');
		...
		if (empty($ids))
		{
			...
			{
				//$this->EE->TMPL->log_item('Calendar:build_calendar() No results, going home');
				return $this->no_results();
			}

			//$this->EE->TMPL->log_item('Calendar:build_calendar() No results, but staying around for the show');
		}
		else
		{
			// -------------------------------------
			...
			if ($each_event != '' OR strpos($this->EE->TMPL->tagdata, '_event_total') !== FALSE)
			{
				while (TRUE)
				{
					//$this->EE->TMPL->log_item('Calendar:build_calendar() Firing up the ol Channel Module to try and process ' . count( $ids ) . ' events.');
					...
					//$this->EE->TMPL->log_item('Calendar:build_calendar() ' . count( $occurrence_ids ) . ' occurrences were found.');
					...
					if ($this->P->value('enable') != FALSE)
					{
						...
						{
							$this->EE->TMPL->tagparams['disable'] = implode('|', array_diff($disable, $this->P->value('enable')));
						}
						else
						{
							$this->EE->TMPL->tagparams['disable'] = implode('|', array_diff($disable, array($this->P->value('enable'))));
						}
					}
					else
					{
						$this->EE->TMPL->tagparams['disable'] = implode('|', $disable);
					}
					...
					//$this->EE->TMPL->tagdata = $this->EE->TMPL->assign_relationship_data( $this->EE->TMPL->tagdata );
					$this->EE->TMPL->tagdata 	= $this->EE->TMPL->assign_relationship_data( $each_event );
					$this->EE->TMPL->var_single 	= array_merge( $this->EE->TMPL->var_single, $this->EE->TMPL->related_markers );
					...
					if ($channel->sql == '')
					{
						//$this->EE->TMPL->log_item('Calendar: Channel query empty');
					...
					if ($channel->query->num_rows == 0)
					{
						//$this->EE->TMPL->log_item('Calendar: Channel Module returned no results');
					...
					{
						//$this->EE->TMPL->log_item('Calendar:build_calendar() Channel module found ' . $channel->query->num_rows . ' results.');
					}
					...
					//$this->EE->TMPL->log_item('Calendar:build_calendar() Beginning event creation process.');
					...
					//$this->EE->TMPL->log_item('Calendar:build_calendar() Event creation resulted in the creation of ' . count( $events ) . ' events.');

					//$this->EE->TMPL->log_item('Calendar:build_calendar() Event creation process finished.');
					...
					if ($each_event == '')
					{
						...
						//$this->EE->TMPL->log_item('Calendar:build_calendar() Skipping further channel module processing because $each_event variable was empty string. ' . count( $entry_data ) . ' events were found and logged into the $entry_data array.');
						break;
					}
					...
					{
						//$this->EE->TMPL->log_item('Calendar:build_calendar() Skipping further channel module processing because there were no calendars connected to the events found.');
						break;
					}
					...
					//$this->EE->TMPL->tagdata = $blargle . '[' . LD . "entry_id" . RD . ']' . $each_event . $bleegle;
					$this->EE->TMPL->tagdata = $blargle .
						'[' . LD . "entry_id" . RD . ']' .
						$this->EE->TMPL->tagdata .
						$bleegle;
					...
					foreach ($channel->query->result as $k => $row)
					{
						...
						//$this->EE->TMPL->log_item('Calendar:build_calendar() We\'re in the channel query result loop. Entry id is ' . $entry_id );
						...
						foreach ($variables as $old => $new)
						{
							if ($old == 'title')
							{
								...
								{
									$channel->query->result[$k][$old]	= $this->EE->typography->parse_type(
										$channel->query->result[$k][$old],
										array(
											'text_format' 	=> 'lite',
											'html_format' 	=> 'none',
					...
					//$this->EE->TMPL->log_item('Calendar: Parsing Weblog stuff');
					...
					//$this->EE->TMPL->log_item('Calendar: Parsing related entries');
		...
		//$this->EE->TMPL->log_item('Calendar:build_calendar() Channel module stuff done');

		// -------------------------------------
		//  Build the calendar
		// -------------------------------------

		//$this->EE->TMPL->log_item('Calendar: Starting to build the calendar');
		...
		//$this->EE->TMPL->log_item('Calendar: Preparing events');
			...
			$this->event_timeframe_total > $this->P->value('event_limit'))
		{
			...
			if ($this->EE->extensions->active_hook('calendar_build_calendar_create_pagination') === TRUE)
			{
				$pagination_data = $this->EE->extensions->call(
		...
		$offset = (
			$this->EE->TMPL->fetch_param('event_offset') ?
				$this->EE->TMPL->fetch_param('event_offset') :
				0
		);
		...
		//$this->EE->TMPL->log_item('Calendar: Beginning date loops');
		...
		for ($y = $start_year; $y <= $end_year; $y++)
		{
			...
			for ($m = $mx; $m <= $my; $m++)
			{
				...
				for ($d = $dx; $d <= $dy; $d++)
				{
					...
					//$this->EE->TMPL->log_item('Calendar: Beginning ' . $ymd . count( $event_array[$ymd] ) . ' events for this day.');
					...
					if ($each_hour != '')
					{
						...
						if ($output_at == 'hour')
						{
							...
							//$this->EE->TMPL->log_item('Calendar: Outputting hour data for '.$ymd);
						}
					}
					...
					if ($each_day != '')
					{
						...
						if ($output_at == 'day')
						{
							$output .= $replace;
							$replace = '';
//$this->EE->TMPL->log_item('Calendar: Outputting day data for '.$ymd);
						}
					...
					if ($each_week != '')
					{
								...
								$this->P->value('pad_short_weeks') === TRUE)
						{
							...
							if ($output_at == 'week')
							{
								$output .= $replace;
								$replace = '';
//$this->EE->TMPL->log_item('Calendar: Outputting week data for '.$ymd);
							}
					...
					if ($each_month != '')
					{
							...
							)
						{
							...
							if ($output_at == 'month')
							{
								$output .= $replace;
								$replace = '';
//$this->EE->TMPL->log_item('Calendar: Outputting month data for '.$ymd);
							}
					...
					if ($each_year != '')
					{
						...
						if ($y != $next_y OR $last_day === TRUE)
						{
							...
							if ($last_day == TRUE)
							{
								...
								if (strpos($this->EE->TMPL->tagdata, 'year_event_total') !== FALSE)
								{
							...
							if ($output_at == 'year')
							{
								$output .= $replace;
								$replace = '';
//$this->EE->TMPL->log_item('Calendar: Outputting year data for '.$ymd);
							}
						}
						else
						{
		...
		//$this->EE->TMPL->log_item('Calendar: All done!');
	...
	public function parse_pagination( $return = '' )
	{
		...
		else
		{
			...
			$this->paginate_data	= $this->EE->functions->prep_conditionals( $this->paginate_data, $pagination_array );
	...
	protected function swap_vars($vars, $tagdata)
	{
		...
		if (isset($vars['single']))
		{
			...
			{
				$tagdata = $this->EE->functions->prep_conditionals($tagdata, array_merge($vars['conditional'], $vars['single']));
			}
			else
			{
				$tagdata = $this->EE->functions->prep_conditionals($tagdata, $vars['single']);
			}
		...
		{
			$tagdata = $this->EE->functions->prep_conditionals($tagdata, $vars['conditional']);
		}
	...
	protected function prep_occurrences_output($tagdata, $data, $include_default = TRUE)
	{
		...
		{
			$entry_info_query = $this->EE->db->query(
				"SELECT 	ct.title, ct.url_title, ct.author_id, ct.status, m.screen_name, m.username
				 ...
				 WHERE		ct.entry_id = " . $this->EE->db->escape_str($data->default_data['entry_id'])
			);
		...
		if (! empty($ids))
		{
			...
			//$this->EE->TMPL_orig = clone $this->EE->TMPL;
			...
			$this->EE->TMPL->tagdata = $this->EE->TMPL->assign_relationship_data( $tagdata );
			$this->EE->TMPL->var_single = array_merge( $this->EE->TMPL->var_single, $this->EE->TMPL->related_markers );
			$this->EE->TMPL->var_single['entry_id'] = 'entry_id';
			...
			//$this->EE->TMPL = $this->EE->TMPL_orig;
		}
		...
		if ($this->parent_method == 'occurrences' AND
			$this->EE->TMPL->fetch_param('reverse') AND
			strtolower($this->EE->TMPL->fetch_param('reverse')) === 'true' )
		{
			krsort($data->dates);
		}
		...
		{
			$sort = ($this->EE->TMPL->fetch_param('sort') AND
					 strtolower($this->EE->TMPL->fetch_param('sort')) === 'DESC') ? 'DESC' : 'ASC';
		...
		$limit = $this->EE->TMPL->fetch_param('occurrences_limit') ?
					$this->EE->TMPL->fetch_param('occurrences_limit') :
					$this->limit;
		...
		$offset = ($this->EE->TMPL->fetch_param('occurrences_offset') ?
					$this->EE->TMPL->fetch_param('occurrences_offset') :
					0);
	...
	{
		$category 	= $split_cat = $this->EE->TMPL->fetch_param('category');
		...
		if ( ! empty ($to_fix))
		{
			$cats = $this->EE->db->query(
				"SELECT cat_id, cat_url_title
				 FROM 	exp_categories
				 WHERE 	cat_url_title
				 IN 	('" . implode("','", $this->EE->db->escape_str(array_keys($to_fix))) . "')"
			);
			...
			$this->EE->TMPL->tagparams['category'] = $category;
		}
	}
	...
	public function permissions_json ()
	{
		...
		return $this->EE->calendar_permissions->permissions_json($group_id);
	}
