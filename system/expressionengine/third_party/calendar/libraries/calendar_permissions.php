...
class Calendar_permissions extends Addon_builder_calendar
{
	...
	public function get_allowed_calendars_for_group ($group_id)
	{
		...
		if (in_array($group_id, $this->EE->calendar_permissions->get_groups_denied_all()))
		{
			return array();
		}
		...
		if (in_array($group_id, $this->EE->calendar_permissions->get_groups_allowed_all()))
		{
	...
	public function get_allowed_entry_ids ($group_id, $not = FALSE)
	{
		...
		$events_table = $this->EE->db->escape_str($this->data->events_table);
		...
		$id_query = $this->EE->db->query(
			"SELECT GROUP_CONCAT(entry_id separator '|') as entry_ids
			 FROM 	{$events_table}
			 WHERE 	calendar_id {$not}
			 IN 	(" . implode(',', $this->EE->db->escape_str($calendar_get_entries)) . ")"
		);
		...
		if ($id_query->num_rows() > 0 AND $id_query->row('entry_ids') != NULL)
		{
			...
			$search = $this->EE->db->escape_str(str_replace('||', '|', $id_query->row('entry_ids')));

			//occurrence IDs are different
			$oc_query = $this->EE->db->query(
				"SELECT GROUP_CONCAT(entry_id separator '|') as entry_ids
	...
	public function get_permissions_preferences ()
	{
		...
		$query = $this->EE->db->query(
			"SELECT preferences
			 ...
			 WHERE  site_id = " . $this->EE->db->escape_str($this->data->get_site_id())
		);
