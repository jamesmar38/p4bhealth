...
class Calendar_ext extends Extension_builder_calendar
{
	...
	public function submit_new_entry_end($id, $data, $ping)
	{
		...
		if ($this->data->channel_is_calendars_channel($data[$channel_id]) === TRUE)
		{
			...
			if (APP_VER > 2.0)
			{
				...
				{
					$this->EE->calendar_permissions->save_calendar_permissions(
						$id,
		...
		elseif ($this->data->channel_is_events_channel($data[$channel_id]) === TRUE)
		{
			...
			if ($this->EE->input->get_post('calendar_parent_entry_id') OR
				$this->EE->input->get_post('calendar_occurrence_id'))
			{
			...
			foreach ($basics as $var)
			{
				...
				elseif ($this->EE->input->post($var) !== FALSE)
				{
					$$var		= $this->EE->input->post($var);
					$data[$var]	= $this->EE->input->post($var);
				}
				else
				{
					$$var		= FALSE;
			...
			if ( ! empty($errors))
			{
				...
				return $this->EE->output->show_user_error('general', $message);
			}
			...
			if ($edit === TRUE)
			{
				...
				if ( ! $this->check_yes($this->EE->input->get_post('has_edited_occurrences')) OR
					 $this->check_yes($this->EE->input->get_post('remove_edited_occurrences')))
				{
	...
	{
		$data = array(
			'occurrence_id'	=> $this->EE->input->post('calendar_occurrence_id'),
			'event_id'		=> $this->EE->input->post('calendar_parent_entry_id'),
			'calendar_id'	=> $this->EE->input->post('calendar_id'),
			'site_id'		=> $this->EE->input->post('site_id'),
			'entry_id'		=> $entry_id,
			'start_date'	=> $this->EE->input->post('start_date'),
			'start_year'	=> substr(
				$this->EE->input->post('start_date'),
				0,
				strlen($this->EE->input->post('start_date')) - 4
			),
			'start_month'	=> substr($this->EE->input->post('start_date'), -4, 2),
			'start_day'		=> substr($this->EE->input->post('start_date'), -2),
			'all_day' 		=> $this->EE->input->post('all_day'),
			'start_time'	=> $this->EE->input->post('start_time'),
			'end_date'		=> $this->EE->input->post('end_date'),
			'end_year'		=> substr(
				$this->EE->input->post('end_date'),
				0,
				strlen($this->EE->input->post('end_date')) - 4
			),
			'end_month'		=> substr($this->EE->input->post('end_date'), -4, 2),
			'end_day'		=> substr($this->EE->input->post('end_date'), -2),
			'end_time'		=> $this->EE->input->post('end_time')
		);
	...
	public function publish_form_headers($a, $b, $c, $channel_id)
	{
			...
			$this->data->channel_is_calendars_channel($channel_id))
		{
			...
			//$this->EE->dsp->extra_header .= $output;
		}
	...
	public function publish_form_start($which, $error, $entry_id, $hidden)
	{
		...
		$data	= array(
			'weblog_id'		=> $this->EE->input->post('weblog_id'),
	...
	public function submit_new_entry_start()
	{
		...
		if ($this->data->channel_is_calendars_channel($this->EE->input->post('weblog_id')) === TRUE OR
			$this->data->channel_is_events_channel($this->EE->input->post('weblog_id')) === TRUE)
		{
			$this->cache['quicksave']	= TRUE;
		}
	}
	...
	public function publish_form_end($channel_id, $is_ee2_ft = FALSE, $ft_data = array())
	{
		...
		if ($this->EE->extensions->last_call !== FALSE AND
			is_string($this->EE->extensions->last_call))
		{
			$output = $this->EE->extensions->last_call;
		}
		...
		if ($this->data->channel_is_calendars_channel($channel_id) === TRUE)
		{
			...
			if (APP_VER > 2.0)
			{
				...
				if ($this->EE->calendar_permissions->enabled())
				{
					$calendar_id 	= $this->EE->input->get('entry_id');
					$calendar_id 	= (
					...
					$output .= $this->view(
						...
						array(
							'permission_data' 		=> $this->EE->calendar_permissions->get_group_permissions($calendar_id),
							'groups_allowed_all' 	=> $this->EE->calendar_permissions->get_groups_allowed_all(),
							'groups_denied_all' 	=> $this->EE->calendar_permissions->get_groups_denied_all(),
		...
		elseif ($this->data->channel_is_events_channel($channel_id) === TRUE)
		{
			...
			if ($id = $this->EE->input->get_post('entry_id') OR
			...
			if ($this->EE->input->get('entry_id') AND
	...
	public function activate_extension()
	{
		if (APP_VER < 2.0)
		{
			return $this->EE->output->show_user_error('general', str_replace('%url%',
															BASE.AMP.'C=modules',
	...
	public function cp_js_end ()
	{
		...
		if ( $this->EE->calendar_permissions->filter_on() != 'disable_link')
		{
			return $js;
		}
	...
	{
		$where = ($this->EE->extensions->last_call) ?
					$this->EE->extensions->last_call : array();
		...
		if ( $this->EE->calendar_permissions->filter_on() != 'search_filter'
			 OR in_array($group_id, $this->EE->calendar_permissions->get_groups_allowed_all()))
		{
			return $where;
		}

		//if they arent allowed to see anything, lets skip the processing and just block the channel
		$allowed_calendars_for_group = $this->EE->calendar_permissions->get_allowed_calendars_for_group($group_id);
		...
		{
			$not_allowed_ids = $this->EE->calendar_permissions->get_denied_entry_ids($group_id);
	...
	public function delete_entries_start ()
	{
		...
		if ($group_id != 1 AND $this->EE->calendar_permissions->enabled())
		{
			...
			foreach ($_POST['delete'] as $key => $delete_id)
			{
				...
				if ( ! $this->EE->calendar_permissions->can_edit_entry($group_id, $delete_id))
				{
	...
	public function update_multi_entries_start ()
	{
		...
		if ($group_id != 1 AND $this->EE->calendar_permissions->enabled())
		{
			...
			{
				//if there are only calendar IDs, lets alert
				if ( ! $this->EE->calendar_permissions->can_edit_entry($group_id, $entry_id))
				{
