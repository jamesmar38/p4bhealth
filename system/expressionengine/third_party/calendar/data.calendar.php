...
class Calendar_data extends Addon_builder_data_calendar
{
	...
	public function get_site_id ()
	{
		...
		else if (	isset($this->EE->TMPL) AND
					is_object($this->EE->TMPL) AND
					! in_array($this->EE->TMPL->fetch_param('site_id'), array(0, '0', FALSE, ''), TRUE) AND
					is_numeric($this->EE->TMPL->fetch_param('site_id')) )
		{
		...
		else if ($this->EE->db->table_exists('exp_calendar_preferences'))
		{
				...
				$query = $this->EE->db->query(
				   "SELECT 	site_id
					...
					WHERE	{$this->sc->db->channel_id} = '" .
							$this->EE->db->escape_str($data['calendar_weblog']) . "'"
				);
	...
	public function get_member_groups ($refresh = FALSE)
	{
		...
		$m_query = $this->EE->db->query(
			"SELECT group_id, group_title
			 FROM 	exp_member_groups
			 WHERE 	group_id != 1
			 AND 	site_id = " . $this->EE->db->escape_str($this->get_site_id())
		);
	...
	public function get_module_preferences($refresh = FALSE)
	{
		...
		if ($this->EE->db->table_exists('exp_calendar_preferences'))
		{
			$sql = 'SELECT 	cp.preferences
					FROM 	exp_calendar_preferences cp
					WHERE 	cp.site_id = "' . $this->EE->db->escape_str($this->get_site_id()) . '"';
	...
	public function update_preferences($data)
	{
		...
		$sql = 'SELECT 	*
				FROM 	exp_calendar_preferences
				WHERE 	site_id = '. $this->EE->db->escape_str($this->get_site_id());
		...
		if ($query->num_rows() == 1)
		{
			$this->EE->db->query(
				$this->EE->db->update_string(
					...
					$data,
					'site_id = '. $this->EE->db->escape_str($this->get_site_id())
				)
			);
		}
		else
		{
			...
			$this->EE->db->query($this->EE->db->insert_string('exp_calendar_preferences', $data));
		}
	...
	public function channel_is_calendars_channel($channel_id)
	{
		...
		$blogname_andor = substr($this->EE->functions->sql_andor_string(
								$this->EE->db->escape_str( $this->calendar_channel_shortname() ),
	...
	public function channel_is_valid_calendar($calendar_id = '')
	{
		...
		$sql = 'SELECT 	calendar_id
				FROM 	exp_calendar_calendars
				WHERE 	calendar_id = "' . $this->EE->db->escape_str($calendar_id) .'"';
	...
	public function channel_is_events_channel($channel_id = 0)
	{
		...
		$sql = "SELECT 	{$this->sc->db->channel_id}
				FROM 	{$this->sc->db->channels}
				WHERE 	{$this->sc->db->channel_name} = '" . $this->EE->db->escape_str(CALENDAR_EVENTS_CHANNEL_NAME) . "'";
	...
	public function add_calendar($id, $site_id, $params)
	{
		...
		$sql = "SELECT 	*
				...
				WHERE 	entry_id = " . $this->EE->db->escape_str($id);
		...
		$this->EE->db->query($this->EE->db->insert_string('exp_calendar_calendars', $data));
		...
		if (! empty($data))
		{
			$this->EE->db->query(
				$this->EE->db->update_string(
					...
					$data,
					'calendar_id = "'. $this->EE->db->escape_str($id) . '"'
				)
			);
			return $id;
		}
	...
	public function update_time_format_field($id, $tf_field)
	{
		$this->EE->db->query(
			$this->EE->db->update_string(
				...
				'entry_id = '. $this->EE->db->escape_str($id)
			)
		);
	}
	...
	{
		$clean_cal_id = $this->EE->db->escape_str($calendar_id);

		//--------------------------------------------
		//	delete event entries
		//--------------------------------------------

		$events_query = $this->EE->db->query(
			"SELECT 		entry_id
	...
	public function add_event($params)
	{
		...
		$this->EE->db->query($this->EE->db->insert_string($this->events_table, $params));
	...
	public function update_event($params)
	{
		...
		$this->EE->db->query($this->EE->db->update_string($this->events_table, $params, 'entry_id = "'. $this->EE->db->escape_str($entry_id) .'"'));
	...
	public function add_imported_event($params)
	{
		...
		$this->EE->db->query($this->EE->db->insert_string('exp_calendar_events_imports', $params));
	...
	public function update_imported_event($params)
	{
		...
		$this->EE->db->query(
			$this->EE->db->update_string(
				'exp_calendar_events_imports',
				$params,
				'import_id = "'. $this->EE->db->escape_str($params['import_id']) .'"'
			)
		);
	...
	public function add_rule($params)
	{
		...
		$this->EE->db->query($this->EE->db->insert_string('exp_calendar_events_rules', $params));
	...
	public function add_occurrence($params)
	{
		...
		$this->EE->db->query($this->EE->db->insert_string('exp_calendar_events_occurrences', $params));
	...
	public function add_exception($params)
	{
		...
		$this->EE->db->query($this->EE->db->insert_string('exp_calendar_events_exceptions', $params));
	...
	public function update_rule($params)
	{
		...
		$this->EE->db->query(
			$this->EE->db->update_string(
				'exp_calendar_events_rules',
				$params,
				'rule_id = "'. $this->EE->db->escape_str($params['rule_id']) .'"'
			)
		);
	...
	public function delete_rule($rule_id)
	{
		...
		$this->EE->db->query(
			'DELETE FROM 	exp_calendar_events_rules
			 WHERE 			rule_id = "'. $this->EE->db->escape_str($rule_id) .'"'
		);
	...
	public function update_occurrence($params)
	{
		...
		$this->EE->db->query(
			$this->EE->db->update_string(
				'exp_calendar_events_occurrences',
				$params,
				'occurrence_id = "'. $this->EE->db->escape_str($params['occurrence_id']) .'"'
			)
		);
	...
	public function update_exception($params)
	{
		...
		$this->EE->db->query(
			$this->EE->db->update_string(
				'exp_calendar_events_exceptions',
				$params,
				'exception_id = "' . $this->EE->db->escape_str($params['exception_id']) .'"'
			)
		);
	...
	public function update_last_date($event_id)
	{
		...
		$sql = "	SELECT 		last_date
					...
					WHERE 		event_id = " . $this->EE->db->escape_str($event_id) ."
		...
		$this->EE->db->query(
			$this->EE->db->update_string(
				...
				"entry_id = ". $this->EE->db->escape_str($event_id)
			)
		);
	}
	...
	public function delete_event($entry_id)
	{
		...
		$this->EE->db->query(
			'DELETE FROM 	exp_calendar_events_rules
			 WHERE 			event_id = "'. $this->EE->db->escape_str($entry_id) .'"'
		);
	...
	public function fetch_occurrences_by_event_id($event_id, $range_start = array(), $range_end = array())
	{
		...
		{
			$start_str = ' AND start_date >= ' . $this->EE->db->escape_str($range_start['year']) .
												 $this->EE->db->escape_str($range_start['month']) .
												 $this->EE->db->escape_str($range_start['day']);
		}

		if (! empty($range_end))
		{
			$end_str = ' AND end_date <= ' . $this->EE->db->escape_str($range_end['year']) .
											 $this->EE->db->escape_str($range_end['month']) .
											 $this->EE->db->escape_str($range_end['day']);
		}
		...
		$sql = '	SELECT 	*
					FROM 	exp_calendar_events_occurrences
					WHERE 	event_id
					IN 		(' . $this->EE->db->escape_str(str_replace('|', ', ', $event_id)) .')'
					. $start_str
					. $end_str;
	...
	public function fetch_full_event_data($entry_id = 0, $url_title = '')
	{
		...
		if ($entry_id > 0)
		{
			$sql .= ' wt.entry_id IN ('. $this->EE->db->escape_str(str_replace('|', ', ', $entry_id)) .')';
		}
		elseif ($url_title != '')
		{
			$sql .= ' wt.entry_id IN ("'. $this->EE->db->escape_str(str_replace('|', '", "', $url_title)) .'")';
		}
		else
		{
			// -------------------------------------
	...
	public function get_calendars_by_site_id($site_id)
	{
		...
		$sql = '	SELECT 	calendar_id
					FROM 	exp_calendar_calendars
					WHERE 	site_id
					IN 		('. $this->EE->db->escape_str($site_id) .')
					AND 	ics_url != ""';
	...
	public function get_calendars_needing_update($ids, $minutes, $status = 'open')
	{
		...
		$data 		= array();
		$ids 		= implode(', ', $ids);
		$statuses	 = '"' . str_replace('|', '", "', $this->EE->db->escape_str($status)) . '"';
		...
		$sql = "SELECT 		cc.calendar_id
				...
				IN 			('" . $this->EE->db->escape_str($ids) . "')
				AND 		wt.status
				IN 			($statuses)
				AND 		cc.ics_url != ''
				AND 		(NOW() - DATE_SUB(NOW(), INTERVAL " . $this->EE->db->escape_str($minutes) . " MINUTE)) > 0 ";
	...
	public function fetch_occurrences_by_event($id)
	{
		...
		$query = $this->EE->db->query('	SELECT occurrence_id, start_date, start_time, end_time
								FROM exp_calendar_events_occurrences
	...
	public function remove_rule($id)
	{
		$this->EE->db->query(
			'DELETE FROM 	exp_calendar_events_rules
			 WHERE 			rule_id = '. $this->EE->db->escape_str($id) . '
			 LIMIT 			1'
		);
	}
	/* END remove_rule() */
	...
	public function remove_occurrence($id)
	{
		$this->EE->db->query(
			'DELETE FROM 	exp_calendar_events_occurrences
			 WHERE 			occurrence_id = '. $this->EE->db->escape_str($id) . '
			 LIMIT 			1'
		);
	}
	...
	public function remove_exception($id)
	{
		$this->EE->db->query(
			'DELETE FROM 	exp_calendar_events_exceptions
			 WHERE 			exception_id = '. $this->EE->db->escape_str($id) . '
			 LIMIT 			1'
		);
	}
	...
	{
		$not_sql  = (! empty($not)) ? ' AND rule_id NOT IN ('. $this->EE->db->escape_str(implode(', ', $not)) .')' : '';

		$this->EE->db->query(
			'DELETE FROM exp_calendar_events_rules
			 WHERE 		 event_id = '. $this->EE->db->escape_str($id) . $not_sql
		);
	}
		...
		if ($query->num_rows() > 0)
		{
			...
			$this->EE->db->query(
				"DELETE FROM 	{$this->sc->db->channel_titles}
				 WHERE 		 	entry_id
				 IN 			(" . implode(',', $ids) . ")
				 AND			entry_id != " . $this->EE->db->escape_str($id)
			);
	...
	{
		$not_sql  = (! empty($not)) ? ' AND exception_id NOT IN ('. $this->EE->db->escape_str(implode(', ', $not)) .')' : '';

		$this->EE->db->query('DELETE FROM exp_calendar_events_exceptions WHERE event_id = '. $this->EE->db->escape_str($id) . $not_sql);
	}
	...
	public function fetch_calendars_basics($site_id, $calendar_id = '', $not = FALSE)
	{
		...
		$sql .= "cc.site_id IN (". str_replace('|', ', ', $this->EE->db->escape_str($site_id)) .")\n";
		...
		{
			$not = ($not === TRUE) ? 'NOT' : '';
			$sql .= "AND cc.calendar_id {$not} IN (". str_replace('|', ', ', $this->EE->db->escape_str($calendar_id)) .")\n";
		}
	...
	public function fetch_calendars_with_events_in_date_range($min = '', $max = '', $calendars = array(), $status = '')
	{
		...
		$min = $this->EE->db->escape_str($min);
		$max = $this->EE->db->escape_str($max);
		...
		if (! empty($calendars))
		{
			...
			if ($calendar_string != '')
			{
				$wheres[] = "calendar_id IN (". $this->EE->db->escape_str($calendar_string) .")\n";
			}

			if ($status_string != '')
			{
				$wheres[] = "ct.status {$status_not} IN ('". $this->EE->db->escape_str($status_string) ."')\n";
			}
	...
	public function fetch_calendars_with_events_in_date_range_by_rule($min = '', $max = '', $calendars = array(), $status_string = '')
	{
		...
		if ($status_string != '')
		{
			$wheres[] = "wt.status {$status_not} IN ('". $this->EE->db->escape_str($status_string) ."')\n";
		}
	...
	public function get_calendar_id_from_name($name = '', $site_id = 0, $not = FALSE)
	{
		...
		$not 		= ($not === TRUE) ? 'NOT' : '';
		$name_str 	= str_replace('|', "', '", $this->EE->db->escape_str($name));
	...
	public function get_calendar_id_by_event_entry_id($entry_id)
	{
		...
		if ($entry_id != '')
		{
			$sql = "SELECT 	ce.calendar_id, ce.entry_id
					FROM 	{$this->events_table} ce
					WHERE 	ce.entry_id
					IN 		(" . $this->EE->db->escape_str($entry_id) .')';
	...
	public function get_event_id_from_name($name, $site_id = 0)
	{
		...
		$sql = "SELECT 	wt.entry_id
				...
				WHERE 	site_id = " . $this->EE->db->escape_str($site_id) . "
				AND 	url_title
				IN 		('" . $this->EE->db->escape_str($name_str) . "')";
	...
	public function fetch_event_ids($params, $category = FALSE)
	{
		...
		$status_string		= str_replace('|', "', '", $this->EE->db->escape_str($params->value('status')));
		$status_not			= '';
		$calendar_not		= '';
		$event_not			= '';
		...
		$dmin				= $this->EE->db->escape_str($dmin);
		$dmax				= $this->EE->db->escape_str($dmax);
		$dtmin				= $this->EE->db->escape_str($dtmin);
		$dtmax				= $this->EE->db->escape_str($dtmax);
		$tmin				= $this->EE->db->escape_str($tmin);
		$tmax				= $this->EE->db->escape_str($tmax);
		...
		if ( $category )
		{
			...
			else
			{
				...
				$cat_q	= $this->EE->db->query(
					"SELECT cat_id, cat_url_title
					 FROM 	exp_categories
					 WHERE 	site_id
					 IN 	(" . str_replace('|', ',' , $params->value('site_id') ) . ")
					 AND	cat_url_title
					 IN  	('" . implode("','" , $this->EE->db->escape_str( $categories )) . "')
		...
		*/
		...
		if (FALSE AND $cat_id != '')
		{
			...
			if (stristr('&', $cat_id))
			{
				...
				if ($cat_not == 'NOT' AND
					$this->check_no($this->EE->TMPL->fetch_param('uncategorized_entries')) === FALSE)
				{
			...
			else
			{
				if (substr($cat_id, 0, 3) == 'not' AND
					$this->check_no($this->EE->TMPL->fetch_param('uncategorized_entries')) === FALSE)
				{
					$cat_sql = $this->EE->functions->sql_andor_string($cat_id, 'cp.cat_id', '', TRUE)." ";
				}
				else
				{
					$cat_sql = $this->EE->functions->sql_andor_string($cat_id, 'cp.cat_id')." ";
				}
			}
		...
		*/
		...
		if ($calendar_string != '')
		{
			$wheres[] = "calendar_id {$calendar_not} IN (". $this->EE->db->escape_str($calendar_string) .")\n";
		}

		if ($event_string != '')
		{
			$wheres[] = "ce.entry_id {$event_not} IN (". $this->EE->db->escape_str($event_string) .")\n";
		}
		...
		if ($event_string != '')
		{
			$wheres[] = "ceo.event_id {$event_not} IN (". $this->EE->db->escape_str($event_string) .")\n";
		}
	...
	public function fetch_event_data($id_string)
	{
		...
		$sql .= "	SELECT 		*
					FROM 		{$this->events_table}
					WHERE 		entry_id
					IN 			(" . $this->EE->db->escape_str($id_string) . ")
					ORDER BY 	start_date ASC,
								end_date ASC";
	...
	public function event_basics($count_only = FALSE, $params = array())
	{
		...
		if ($this->EE->input->get_post('calendar'))
		{
			$wheres[] = 'ce.calendar_id = ' . $this->EE->db->escape_str($this->EE->input->get_post('calendar'));
		}

		if ($this->EE->input->get_post('status'))
		{
			$wheres[] = 'wt.status = "' . $this->EE->db->escape_str($this->EE->input->get_post('status')) . '"';
		}

		if (isset($params['recurs']))
		{
			$wheres[] = 'ce.recurs = "' . $this->EE->db->escape_str($params['recurs']) . '"';
		}
		elseif ($this->EE->input->get_post('recurs'))
		{
			$wheres[] = 'ce.recurs = "' . $this->EE->db->escape_str($this->EE->input->get_post('recurs')) . '"';
		}

		if ($this->EE->input->get_post('date'))
		{
			$formatted_date = $this->format_input_date($this->EE->input->get_post('date'));
			$date 			= $formatted_date['ymd'];

			if ($this->EE->input->get_post('date_direction'))
			{
				$dirs 		= array(
					'greater'	=> '>=',
					'less'		=> '<=',
				...
				$dir 		= (array_key_exists($this->EE->input->get_post('date_direction'), $dirs)) ?
									$dirs[$this->EE->input->get_post('date_direction')] : '=';

				$wheres[] 	= 'ce.start_date ' . $this->EE->db->escape_str($dir) . ' ' . $this->EE->db->escape_str($date);
			}
			else
			{
				$wheres[] 	= 'ce.start_date ' . '>= ' . $this->EE->db->escape_str($date);
			}
		}

		$orderby = ($this->EE->input->get_post('orderby')) ?
					$this->EE->db->escape_str($this->EE->input->get_post('orderby')) : 'wt.title';
		...
		$sort 		= ($this->EE->input->get_post('sort')) ?
						$this->EE->db->escape_str($this->EE->input->get_post('sort')) 	: 'ASC';
		$offset 	= ($this->EE->input->get_post('offset')) ?
						$this->EE->db->escape_str($this->EE->input->get_post('offset')) 	: 0;
		$limit 		= (is_numeric($this->EE->input->get_post('limit'))) ?
						$this->EE->db->escape_str($this->EE->input->get_post('limit')) 	: FALSE;
	...
	public function get_tz_offset_field_id()
	{
		...
		$sql .= "SELECT field_id
				 ...
				 WHERE 	site_id = " . $this->EE->db->escape_str($this->get_site_id()) . "
				 AND 	field_name = '" . $this->EE->db->escape_str(CALENDAR_CALENDARS_FIELD_PREFIX.'tz_offset') . "'
				 LIMIT 	1 ";
	...
	public function get_time_format_field_id()
	{
		...
		$sql = "SELECT 	field_id
				FROM 	{$this->sc->db->channel_fields}
				WHERE 	site_id = " . $this->EE->db->escape_str($this->get_site_id()) . "
				AND 	field_name = '" . $this->EE->db->escape_str(CALENDAR_CALENDARS_FIELD_PREFIX . 'time_format') . "'
				LIMIT 	1";
	...
	public function get_ics_url_field_id()
	{
		...
		$sql = "SELECT 	field_id
				FROM 	{$this->sc->db->channel_fields}
				WHERE 	site_id = " . $this->EE->db->escape_str($this->get_site_id()) . "
				AND 	field_name = '" . $this->EE->db->escape_str(CALENDAR_CALENDARS_FIELD_PREFIX . 'ics_url') . "'
				LIMIT 	1";
														...
														$end_time = NULL)
	{
		...
		{
			$sql .= 'AND ceo.start_date = "'. $this->EE->db->escape_str($start_date) . '"
					 AND ceo.end_date = "'. $this->EE->db->escape_str($end_date) .'" ';
		}

		if ($start_time AND $end_time)
		{
			$sql .= 'AND ceo.start_time = "'. $this->EE->db->escape_str($start_time) .'"
					 AND ceo.end_time = "'. $this->EE->db->escape_str($end_time) .'"';
		}
