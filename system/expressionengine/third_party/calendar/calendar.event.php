...
class Calendar_event extends Calendar
{
	...
	public function __construct($data, $start = array(), $end = array(), $limit = 0)
	{
		...
		if ($this->EE->extensions->active_hook('calendar_event_end') === TRUE)
		{
			$this->EE->extensions->call('calendar_event_end', $this);
	...
	public function add_dates_using_rule($rule)
	{
		...
		if (isset($this->EE->TMPL) AND
			...
			! in_array($this->EE->TMPL->fetch_param('prior_occurrences_limit'), array(FALSE, ''), TRUE) AND
			is_numeric($this->EE->TMPL->fetch_param('prior_occurrences_limit')) AND
			$this->EE->TMPL->fetch_param('prior_occurrences_limit') > 0)
		{
			$prior_occurrences_limit = $this->EE->TMPL->fetch_param('prior_occurrences_limit');
		}
	...
	public function check_range_hours ($add_it = TRUE)
	{
		...
		$has_tmpl = isset($this->EE->TMPL) AND is_object($this->EE->TMPL);
		...
		if ( ! $has_tmpl OR ! $this->check_yes(
				$this->EE->TMPL->fetch_param('legacy_add_recurring')
			)
		)
		{
			if ($has_tmpl AND $this->check_yes(
					$this->EE->TMPL->fetch_param('inclusive_time_range')
				))
			{
				return (
