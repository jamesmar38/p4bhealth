...
class Calendar_upd extends Module_builder_calendar
{
	...
	public function install()
	{
		...
		$sql = "SELECT		wf.field_id, wf.field_name, wf.group_id
				...
				AND			fg.site_id = '" . $this->EE->db->escape_str($this->data->get_site_id()) . "'";
		...
		$sql = "SELECT 	w.*
				...
				AND 	w.site_id = '" .
						$this->EE->db->escape_str($this->data->get_site_id()) . "'";
		...
		$sql[] = $this->EE->db->insert_string('exp_modules', $insert_data);
		...
		if ($calendars_field_group == '')
		{
			...
			$this->EE->db->query($this->EE->db->insert_string('exp_field_groups', $data));
			$calendars_field_group = $this->EE->db->insert_id();
		}
		...
		if ($events_field_group == '')
		{
			...
			$this->EE->db->query($this->EE->db->insert_string('exp_field_groups', $data));
			$events_field_group = $this->EE->db->insert_id();
		}
		...
		if (empty($calendars_weblogs))
		{
			...
			else
			{
				...
				$errors = array();
				if (isset($this->EE->api_channel_structure->errors) AND ! empty($this->EE->api_channel_structure->errors))
				{
					$errors = $this->EE->api_channel_structure->errors;
					$this->EE->api_channel_structure->errors = array();
				}

				$this->EE->api_channel_structure->create_channel($_POST);

				// Part 2 of the fix
				$this->EE->api_channel_structure->errors = $errors;
			}
		}
	...
	public function update($current = '')
	{
		...
		if ($this->version_compare($this->database_version(), '<', '1.0.0.b2'))
		{
			...
			$sql[] = '	UPDATE exp_calendar_preferences
						SET preferences = "'. $this->EE->db->escape_str($this->_default_preferences()) .'"
						WHERE preferences = ""
						';
		}
			...
			if ( ! isset($prefs['calendar_weblog']) OR $prefs['calendar_weblog'] === '' )
			{
				$query = $this->EE->db->query(
					"SELECT {$this->sc->db->channel_id}
					 FROM 	{$this->sc->db->channels}
					 WHERE 	{$this->sc->db->channel_name} = '" .
								$this->EE->db->escape_str(CALENDAR_CALENDARS_CHANNEL_NAME_DEFAULT) . "'"
				);
			...
			if ( ! isset($prefs['event_weblog']) OR $prefs['event_weblog'] === '')
			{
				$query = $this->EE->db->query(
					"SELECT {$this->sc->db->channel_id}
					 FROM 	{$this->sc->db->channels}
					 WHERE 	{$this->sc->db->channel_name} = '" .
								$this->EE->db->escape_str(CALENDAR_EVENTS_CHANNEL_NAME_DEFAULT) . "'"
				);
		...
		if ($this->version_compare($this->database_version(), '<', '1.5.2'))
		{
			...
			$where = implode(',', $this->EE->db->escape_str(array($prefs['calendar_weblog'], $prefs['event_weblog'])));
		...
		{
			if (APP_VER >= 2.0 AND
				$this->EE->db->table_exists('exp_calendar_permissions_preferences') === FALSE)
			{
				$newest_prefs = TRUE;
	...
	{
		$cal_query = $this->EE->db->query(
			"SELECT {$this->sc->db->channel_id} AS channel_id
			 FROM 	{$this->sc->db->channels}
			 WHERE 	{$this->sc->db->channel_name} = '" . $this->EE->db->escape_str(CALENDAR_CALENDARS_CHANNEL_NAME_DEFAULT) . "'"
		);

		$event_query = $this->EE->db->query(
			"SELECT {$this->sc->db->channel_id} AS channel_id
			 FROM 	{$this->sc->db->channels}
			 WHERE 	{$this->sc->db->channel_name} = '" .
			 	$this->EE->db->escape_str(CALENDAR_EVENTS_CHANNEL_NAME_DEFAULT) . "'"
	...
	public function update_channel_fields()
	{
		...
		$ft_settings = $this->EE->api_channel_fields->apply('save_settings', array($_POST));
		...
		foreach($native as $key)
		{
			...
			{
				$native_settings[$key] = $this->EE->config->item('site_id');
			}
		...
		$this->EE->db->query($this->EE->db->insert_string('channel_fields', $native_settings));
		...
		$this->EE->db->query(
			"UPDATE exp_channel_data
			 SET field_ft_".$insert_id." = '".$this->EE->db->escape_str($native_settings['field_fmt'])."'"
		);
		...
		$this->EE->api_channel_fields->apply('post_save_settings', array($_POST));
