...
class Calendar_mcp extends Module_builder_calendar
{
	...
	public function view_events($message='')
	{
		...
		if ($this->EE->input->post('toggle') !== FALSE AND $this->EE->input->post('delete_confirm') != '')
		{
			return $this->delete_events_confirm();
		}
		elseif ($this->EE->input->post('delete') !== FALSE AND is_array($this->EE->input->post('delete')) AND count($this->EE->input->post('delete')) > 0)
		{
			$this->delete_events();
		}
		...
		$this->cached_vars['calendar']	= ($this->EE->input->post('calendar')) ? $this->EE->input->post('calendar') : '';
		$this->cached_vars['status']	= ($this->EE->input->post('status')) ? $this->EE->input->post('status') : '';
		$this->cached_vars['recurs']	= ($this->EE->input->post('recurs')) ? $this->EE->input->post('recurs') : '';
		$this->cached_vars['date']		= ($this->EE->input->post('date')) ? $this->EE->input->post('date') : '';
		$this->cached_vars['direction']	= ($this->EE->input->post('date_direction')) ? $this->EE->input->post('date_direction') : '';
		$this->cached_vars['orderby']	= ($this->EE->input->post('orderby')) ? $this->EE->input->post('orderby') : 'title';
		$this->cached_vars['sort']		= ($this->EE->input->post('sort')) ? $this->EE->input->post('sort') : 'ASC';
		$this->cached_vars['offset']	= ($this->EE->input->get_post('offset')) ? $this->EE->input->get_post('offset') : 0;
		$this->cached_vars['limit']		= ($this->EE->input->get_post('limit')) ? $this->EE->input->get_post('limit') : 100;
		...
		$this->cached_vars['paginate']	= $this->EE->pagination->create_links();
	...
	public function delete_events_confirm($message = '')
	{
		...
		$this->cached_vars['question']	= str_replace('{COUNT}', count($this->EE->input->post('toggle')), lang('delete_events_question'));
		$this->cached_vars['delete']	= lang('delete');
		$this->cached_vars['items']		= $this->EE->input->post('toggle');
	...
	public function delete_events()
	{
		...
		if ($this->EE->extensions->active_hook('calendar_delete_events') === TRUE)
		{
			$edata = $this->EE->extensions->call('calendar_delete_events', $this);
		...
		if ( ! $this->EE->cp->allowed_group('can_delete_self_entries') AND
			 ! $this->EE->cp->allowed_group('can_delete_all_entries'))
		{
			show_error(lang('unauthorized_access'));
		}
		...
		if ($this->EE->extensions->active_hook('delete_entries_start') === TRUE)
		{
			$edata = $this->EE->extensions->call('delete_entries_start');
			if ($this->EE->extensions->end_script === TRUE) return;
		}
		//
		// -------------------------------------------
		...
		$res = $this->EE->api_channel_entries->delete_entry($this->EE->input->post('delete'));
	...
	public function edit_occurrences($message='')
	{
		...
		if ( $this->EE->input->get_post('event_id') === FALSE ) return FALSE;

		// -------------------------------------
		//  filtering input data
		// -------------------------------------

		$this->cached_vars['event_id']		= $event_id 	= $this->EE->input->get_post('event_id');
		$this->cached_vars['status']		= $status 		= ($this->EE->input->get_post('status')) ?
																$this->EE->input->get_post('status') : '';
		$this->cached_vars['date']			= $input_date	= ($this->EE->input->get_post('date')) ?
																$this->EE->input->get_post('date') : '';
		$this->cached_vars['direction']		= $direction 	= ($this->EE->input->get_post('date_direction')) ?
																$this->EE->input->get_post('date_direction') : '';
		$this->cached_vars['orderby']		= $orderby		= ($this->EE->input->get_post('orderby')) ?
																$this->EE->input->get_post('orderby') : 'start_date';
		$this->cached_vars['sort']			= $sort			= ($this->EE->input->get_post('sort')) ?
																$this->EE->input->get_post('sort') : 'ASC';
		$this->cached_vars['offset']		= $offset 		= is_numeric($this->EE->input->get_post('offset')) ?
																$this->EE->input->get_post('offset') : 0;
		$this->cached_vars['limit']			= $limit 		= ($this->EE->input->get_post('limit')) ?
																$this->EE->input->get_post('limit') : 50;
		$this->cached_vars['occurrences_limit']	= $occurrences_limit = (
			$this->EE->input->get_post('occurrences_limit')
		) ? $this->EE->input->get_post('occurrences_limit') : 100;
		...
		foreach ($events[$event_id]->dates as $ymd => $times)
		{
			...
			foreach ($times as $time => $data)
			{
				...
				if ( ! in_array($this->EE->input->get_post('status'), array(FALSE, ''), TRUE) AND
					 $event_view['ostatus'] !== $this->EE->input->get_post('status'))
				{
					continue;
				}
	...
	public function permissions ($message='')
	{
		...
		$this->cached_vars['groups_allowed_all'] 	= $this->EE->calendar_permissions->get_groups_allowed_all();
		$this->cached_vars['groups_denied_all']  	= $this->EE->calendar_permissions->get_groups_denied_all();
		$this->cached_vars['permissions_enabled'] 	= $this->EE->calendar_permissions->enabled();
		$this->cached_vars['filter_on'] 			= $this->EE->calendar_permissions->filter_on();
		...
		$calendar_permissions 		= $this->EE->calendar_permissions->get_group_permissions();
	...
	public function save_permissions ()
	{
		...
		$this->EE->calendar_permissions->save_permissions($this->EE->security->xss_clean($_POST));
	...
	public function view_preferences($message='')
	{
		...
		$menu = $this->EE->localize->timezone_menu(
			$this->EE->config->item($this->EE->config->item('site_short_name') . '_timezone')
		);
	...
	public function calendar_module_update()
	{
		...
		if ($U->update() !== TRUE)
		{
			$this->EE->functions->redirect($this->base.'&msg=update_failure');
		}
		else
		{
			$this->EE->functions->redirect($this->base.'&msg=update_successful');
		}
	}
