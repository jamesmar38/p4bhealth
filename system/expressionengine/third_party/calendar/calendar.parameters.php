...
class Calendar_parameters extends Addon_builder_calendar {
	...
	public function fetch_dynamic_parameters ()
	{
		...
		foreach (explode('|', $this->EE->TMPL->fetch_param('dynamic_parameters')) as $val)
		{
	...
	public function add_parameter ($name, $details = array())
	{
		...
		{
			$valid_values = $this->EE->extensions->call($hook, $valid_values);
		...
		if ($this->EE->extensions->active_hook($hook) === TRUE)
		{
			$default = $this->EE->extensions->call($hook, $default);
			if ($this->EE->extensions->end_script === TRUE) return;
		}
	...
	public function fetch_value ($name, $methods = array('tmpl'))
	{
		...
		foreach ($methods as $method)
		{
			...
			{
				case 'tmpl' :
					$value = $this->EE->TMPL->fetch_param($name);
					break;

				case 'GET' :
					$value = $this->EE->input->get($name);
					break;

				case 'POST' :
					$value = $this->EE->input->post($name);
					break;

				case 'cookie' :
					$value = $this->EE->input->cookie($name);
					break;
				...
				default :
					...
					if ($this->EE->extensions->active_hook($hook) === TRUE)
					{
						$value = $this->EE->extensions->call($hook, $method, $name);
						if ($this->EE->extensions->end_script === TRUE) return;
					}
	...
	public function validate_type ($value, $details)
	{
		switch($details['type'])
		{
			...
			default :
				...
				if ($this->EE->extensions->active_hook($hook) === TRUE)
				{
					return $this->EE->extensions->call($hook, $value, $details);
				}
