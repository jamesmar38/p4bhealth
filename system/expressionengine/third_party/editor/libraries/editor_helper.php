...
class Editor_helper
{
	...
	public function get_editor_json($settings=array())
	{
		...
		$act_url = $this->EE->editor_helper->get_router_url('url', 'ACT_file_upload');
		...
		if (isset($settings['upload_service']) === TRUE && $settings['upload_service'] == 's3')
		{
			...
			$js->uploadFields->success_action_redirect = $this->EE->editor_helper->get_router_url('url', 'ACT_file_upload');
			...
			$pol = '{	"expiration": "'.date('Y-m-d', strtotime('+1 week')).'T12:00:00.000Z",
						"conditions": [
							...
							{"bucket": "'.$bucket.'"},
							["starts-with", "$key", ""],
							{"success_action_redirect": "'.$this->EE->editor_helper->get_router_url('url', 'ACT_file_upload').'" }
						]
					}';
...
//		$js = $this->generate_json($js);
	...
	public function get_theme_url()
	{
        ...
        if (isset($_SERVER['HTTP_REFERER']) == true && strpos($_SERVER['HTTP_REFERER'], 'https://') !== false) {
        ...
        if (! defined('EDITOR_THEME_URL')) {
            define('EDITOR_THEME_URL', $theme_url . $this->package_name.'/');
        }

        return EDITOR_THEME_URL;
	}

	// ********************************************************************************* //

	public function get_router_url($type='url', $method='ACT_file_upload')
	{
		// Do we have a cached version of our ACT_ID?
		if (isset($this->EE->session->cache['editor']['router_url'][$method]['ACT_ID']) == FALSE)
		{
			$this->EE->db->select('action_id');
			$this->EE->db->where('class', 'Editor');
			$this->EE->db->where('method', $method);
			$query = $this->EE->db->get('actions');
			$ACT_ID = $query->row('action_id');
		}
		else $ACT_ID = $this->EE->session->cache['editor']['router_url'][$method]['ACT_ID'];
		...
		if ($type == 'url')
		{
			...
			$over = $this->EE->config->item('editor_act_url');
			...
			if (isset($this->EE->session->cache['editor']['router_url'][$method]['URL']) == TRUE) return $this->EE->session->cache['editor']['router_url'][$method]['URL'];
			$this->EE->session->cache['editor']['router_url'][$method]['URL'] = $ajax_url;
			return $this->EE->session->cache['editor']['router_url'][$method]['URL'];
		}
   	...
   	public function mcp_js_css($type='', $path='', $package='', $name='', $theme_url=false)
	{
        ...
        if ($type == 'gjs') {
            if (isset($this->EE->session->cache['inline_js'][$this->package_name]) === false ) {
                ...
                $THEME_url = $url;
                $MCP_BASE = (defined('BASE')) ? BASE.AMP.'C=addons_modules'.AMP.'M=show_module_cp'.AMP.'module=editor' : '';

                $js = " var Editor = Editor ? Editor : new Object();
						Editor.AJAX_URL = '{$AJAX_url}';
						Editor.THEME_URL = '{$THEME_url}';
						Editor.site_id = '{$this->site_id}';
						Editor.MCP_BASE = '{$MCP_BASE}';
                    ";
